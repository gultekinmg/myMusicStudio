...
var pReduce = createCommonjsModule(function (module) {
    ...
    module.exports = (function (iterable, reducer, initVal) { return new Promise(function (resolve, reject) {
        ...
        var next = function (total) {
            ...
            if (el.done) {resolve(total);return;
            }
            Promise.all([total,el.value]).then(function (value) {next(reducer(value[0], value[1], i++));
            }).catch(reject);
        };
        next(initVal);
    }); });
...
var EmbedJS = function EmbedJS(options) {
	...
	EmbedJS.prototype.load = function load () {var this$1 = this;
			this.options.plugins.forEach(function (p) { return p.onLoad && p.onLoad(this$1.options); });
	};
	EmbedJS.prototype.render = function render () { var this$1 = this;
			return new Promise(function ($return, $error) {
					...
					if (!isElementPresent(this$1.options)) {return $error(new Error("You haven't passed the input as an element."));}
					if (isDom(input) && input.classList.contains("ejs-applied")) {options = this$1.options;return $If_1.call(this$1);
					} else {return this$1.text().then(function ($await_2) {
									try {
											options = $await_2;
									...
									} catch ($boundEx) {return $error($boundEx);}
							}, $error);
					}
					function $If_1() {this.load();return $return(options);}
			});
	};
	EmbedJS.prototype.destroy = function destroy () {
			var ref = this.options;
					...
					var input = ref.input;
					var target = ref.target;
			if (!isElementPresent(this.options)) {throw new Error("You haven't passed the input as an element.");}

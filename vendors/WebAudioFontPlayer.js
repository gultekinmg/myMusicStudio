...
(
	{///WebAudioFont
		...
		function WebAudioFontLoader(player) {this.player = player;this.cached = [];
			this.startLoad = function (audioContext, filePath, variableName) {
				if (window[variableName]) { return; }
				for (var i = 0; i < this.cached.length; i++) {if (this.cached[i].variableName == variableName) {return;}}
				this.cached.push({filePath: filePath,variableName: variableName});
			...
			this.decodeAfterLoading = function (audioContext, variableName) {
				var me = this;
				this.waitOrFinish(variableName, function () {me.player.adjustPreset(audioContext, window[variableName]);});
			};
			this.waitOrFinish = function (variableName, onFinish) {
				if (window[variableName]) {onFinish();} else {var me = this;setTimeout(function () {me.waitOrFinish(variableName, onFinish);}, 111);}
			};
			this.loaded = function (variableName) {
				if (!(window[variableName])) {return false;}
				var preset = window[variableName];
				for (var i = 0; i < preset.zones.length; i++) {if (!(preset.zones[i].buffer)) {return false;}}
				return true;
			};
			...
			this.instrumentTitles = function () { /// instrumentTitles
			...
			this.instrumentKeys = function () { /// 0-1274 instrument files
			...
			this.instrumentInfo = function (n) {
				...
				return {
					variable: '_tone_' + key,
					url: '../_webaudiofontdata/sound/' + key + '.js',
			...
			this.drumInfo = function (n) {
				...
				return {
					variable: '_drum_' + key,
					url: '../../_sf2/webaudiofontdata/_SF2_file.js/sound/128' + key + '.js',
					pitch: p,

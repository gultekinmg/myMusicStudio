$(function() {
	/// VARIBLES
	crashCymbolAll = $('#Crash');crashCymbol = $('#Crash-Cymbol');crashAudio = $('#Crash-Audio');
	rightTomDrumAll = $('#Tom-Right-All');rightTomDrum = $('#Tom-Right-Drum');smallTomAudio = $('#Small-Rack-Tom-Audio');
	leftTomDrumAll = $('#Tom-Left-All');leftTomDrum = $('#Tom-Left-Drum');bigTomAudio = $('#Big-Rack-Tom-Audio');
	floorTomDrumAll = $('#Floor-Tom');floorTomAudio = $('#Floor-Tom-Audio');
	snareDrumAll = $('#Snare');snareDrum = $('#Snare-Drum');snareAudio = $('#Snare-Audio');
	kickDrumAll = $('#Kick');kickAudio = $('#Kick-Audio');
	hiHatAll = $('#Hi-Hat'); hiHatTop = $('#Hi-Hat-Top'); hiHatBottom = $('#Hi-Hat-Bottom'); hiHatStandTop = $('#Hi-Hat-Stand-Top');
	hiHatClosedAudio = $('#Hi-Hat-Closed-Audio'); hiHatOpenAudio = $('#Hi-Hat-Open-Audio');
	...
	// Crash play
	window.crash = function() {crashtl.restart();crashtl.play();
	...
	crashCymbolAll.on("touchstart click", function() {
		if(!clickTouchCrashDone) {	clickTouchCrashDone = true;
		setTimeout(function() {clickTouchCrashDone = false;	}, 100); crash(); return false;}
	});
	// Right tom play
	window.rightTom = function() {rightTomtl.restart();rightTomtl.play();
	...
	rightTomDrumAll.on("touchstart click", function() {
		if(!clickTouchRTDrumDone) { clickTouchRTDrumDone = true;
			setTimeout(function() {		clickTouchRTDrumDone = false;	}, 100); rightTom(); return false;
		}
	});
	// Left tom play
	window.leftTom = function(){leftTomtl.restart();leftTomtl.play();
	...
	leftTomDrumAll.on("touchstart click", function() {
		if(!clickTouchLTDrumDone) { clickTouchLTDrumDone = true;
			setTimeout(function() {		clickTouchLTDrumDone = false;	}, 100); leftTom(); return false;
		}
	});
	// Floor tom play
	...
	floorTomDrumAll.on("touchstart click", function() {
		if(!clickTouchTDrumDone) { clickTouchTDrumDone = true;
			setTimeout(function() {clickTouchTDrumDone = false;}, 100); floorTom(); return false;
		}
	});
	// Snare play
	...
	snareDrumAll.on("touchstart click", function() {
		if(!clickTouchSnareDone) { clickTouchSnareDone = true;
			setTimeout(function() {clickTouchSnareDone = false;}, 100); snare(); return false;
		}
	});
	// Kick play
	...
	kickDrumAll.on("touchstart click", function() {
		if(!clickTouchKickDone) { clickTouchKickDone = true;
			setTimeout(function() {clickTouchKickDone = false; }, 100); kick(); return false;
		}
	});
	
	/// Hi-hat play
	// hi-hat stand up/down svg
	var hiHatUp = new TimelineMax({paused: true});	hiHatUp.to(hiHatStandTop, 0.3, {y:"-5%", ease: Bounce.easeOut});
	// hi-hat counter and tl stuff
	hiHati = 0;
	window.hiHat = function() {
		++hiHati;
		hiHattl.restart();	hiHattl.play();		
		var hiHatClosedAudioEl = hiHatClosedAudio.get(0);
		var hiHatOpenAudioEl = hiHatOpenAudio.get(0);		
		if (hiHati < 8) {hiHatUp.reverse(); hiHatClosedAudioEl.currentTime = 0;hiHatClosedAudioEl.play();
		} else {hiHatUp.play(); hiHatOpenAudioEl.currentTime = 0;hiHatOpenAudioEl.play();hiHati = 0;}
	}
	window.hiHatdown = function() {
		hiHattl.restart();	hiHattl.play();		
		var hiHatClosedAudioEl = hiHatClosedAudio.get(0);
		hiHatUp.reverse(); hiHatClosedAudioEl.currentTime = 0;hiHatClosedAudioEl.play();
	}
	window.hiHatU = function() {
		hiHattl.restart();	hiHattl.play();
		var hiHatOpenAudioEl = hiHatOpenAudio.get(0);
		hiHatUp.play();
		hiHatOpenAudioEl.currentTime = 0;
		hiHatOpenAudioEl.play();hiHati = 0;
	}
	// Do the hi-hat stuff when clicked/touched
	var clickTouchHitHatDone = false;
	hiHatTop.on("touchstart click", function() {
		if(!clickTouchHitHatDone) { clickTouchHitHatDone = true;
			setTimeout(function() {clickTouchHitHatDone = false;}, 100); hiHatdown(); return false;
		}
	});
	hiHatBottom.on("touchstart click", function() {
		if(!clickTouchHitHatDone) { clickTouchHitHatDone = true;
			setTimeout(function() {clickTouchHitHatDone = false;}, 100); hiHatU(); return false;
		}
	});
	...
	function animateKey(key) {
		keytl = new TimelineMax({paused: true});
		keytl.to(key, 0.1, {scale: 1.1, transformOrigin: "50% 50%", ease: Expo.easeOut})
		.to(key, 0.4, {scale: 1, transformOrigin: "50% 50%", ease: Elastic.easeOut});keytl.restart();keytl.play();
	}
	// Show/hide keys
	$('#keys-btn').click(function() {allKeys.fadeToggle();	});
	// Key triggers
	document.onkeydown = function(e) {
		thisKeyID = 'Key-' + e.keyCode;
		thisKey = $('#' + thisKeyID);
		switch (e.keyCode) {
			case 74:hiHat();animateKey(thisKey);break;
			case 72:snare();animateKey(thisKey);break;
			case 66:kick();animateKey(thisKey);break;
			case 71:floorTom();animateKey(thisKey);break;
			case 70:crash();animateKey(thisKey);break;
			case 84:leftTom();animateKey(thisKey);break;
			case 89:rightTom();animateKey(thisKey);break;
		}
	};
	...
	$('#sequencer-visible-btn').click(function () {
		...
		if (sequencerVisible === false) {sequencerVisible = true;}  else {sequencerVisible = false;}
	...
	function sequencer () {
	  ...
	  if ( beat < (rowLength - 1) ) {++beat;} else {beat = 1;}
	}
	...
	$('#sequencer-active-btn').click(function () {
		...
		if (sequencerOn === false) {intervalId = window.setInterval(sequencer, interval);sequencerOn = true;
		} else {window.clearInterval(intervalId);sequencerOn = false;
		}
	});
	// Tempo varibles
	bpm = 150;
	...
	$('#bpm-increase-btn').click(function() {
		if ( bpm < 300 ) {bpm = parseInt($('#bpm-indicator').val());bpm += 10;interval = 60000 / bpm;$('#bpm-indicator').val(bpm);
			if(sequencerOn === true) {setTempo();}
		}
	});
	//Decrease tempo
	$('#bpm-decrease-btn').click(function() {
		if ( bpm > 100 ) {bpm = parseInt($('#bpm-indicator').val());bpm -= 10;interval = 60000 / bpm;$('#bpm-indicator').val(bpm);
			if(sequencerOn === true) {setTempo();}
		}
	});
	...
	$('#sequencer-visible-btn').one('click', function() {
		$("audio").each(function(i) { this.play(); this.pause();
		});
	});
});